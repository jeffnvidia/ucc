...
get_peer(const ucc_tl_ucp_task_t *task, ucc_rank_t rank, ucc_rank_t size,
         ucc_rank_t step, int is_send)
{
    const ucc_tl_ucp_team_t *team = TASK_TEAM(task);

    if (task->alltoall_pairwise.use_node_interleaved) {
    ...
    while ((task->tagged.send_posted < gsize ||
        ...
        while ((task->tagged.recv_posted < gsize) &&
               ((task->tagged.recv_posted - task->tagged.recv_completed) <
                nreqs)) {
            peer = get_peer(task, grank, gsize, task->tagged.recv_posted, 0);
        ...
        while ((task->tagged.send_posted < gsize) &&
               ((task->tagged.send_posted - task->tagged.send_completed) <
                nreqs)) {
            peer = get_peer(task, grank, gsize, task->tagged.send_posted, 1);
...
ucc_status_t ucc_tl_ucp_alltoall_pairwise_init_common(ucc_tl_ucp_task_t *task)
{
    ...
    size_t             data_size, peer_size;

    task->super.post     = ucc_tl_ucp_alltoall_pairwise_start;
    task->super.progress = ucc_tl_ucp_alltoall_pairwise_progress;

    peer_size = (size_t)(args->src.info.count / UCC_TL_TEAM_SIZE(team)) *
                ucc_dt_size(args->src.info.datatype);
    task->alltoall_pairwise.use_node_interleaved =
        team->alltoall_node_interleaved.available &&
        (team->cfg.alltoall_pairwise_schedule ==
         ...
         (team->cfg.alltoall_pairwise_schedule ==
          ...
          ucc_tl_ucp_alltoall_auto_uses_node_interleaved(
              UCC_TL_TEAM_SIZE(team), peer_size, args->src.info.mem_type,
              args->dst.info.mem_type)));
    if (task->alltoall_pairwise.use_node_interleaved) {
        tl_debug(UCC_TL_UCP_TEAM_LIB(team),
                 "alltoall node-interleaved ring selected: size %u peer "
                 "size %zu",
                 UCC_TL_TEAM_SIZE(team), peer_size);
    }
